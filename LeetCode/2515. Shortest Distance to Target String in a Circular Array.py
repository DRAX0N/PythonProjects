...
class Solution:
    def closestTarget(self, words: List[str], target: str, startIndex: int) -> int:
        ...
        for index in range(n):
            if words[(startIndex + index)%n] == target:
                    min_dist = min(min_dist, index)
            if words[(startIndex - index)%n] == target:
                min_dist = min(min_dist, index+1)
        return min_dist
...
if __name__ == "__main__":
    ...
    target = "hello"
    startIndex = 1
    print(Solution().closestTarget(words, target, startIndex)) # 1
    words = ["a","b","leetcode"]
    target = "leetcode"
    startIndex = 0
    print(Solution().closestTarget(words, target, startIndex)) # 1
    ...
    target ="zemkwvqrww"
    startIndex = 8
    print(Solution().closestTarget(words, target, startIndex)) # 4