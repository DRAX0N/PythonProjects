...
class Solution:
    def mapWordWeights(self, words: List[str], weights: List[int]) -> str:
        ...
        for word in words:
            res = 0
            for ch in word:
                res += weights[(ord(ch)-ord("a"))]
            result.append(chr(abs(res%modulo - 25)+ord("a")))
        return "".join(result)
...
if __name__ == "__main__":
    s = Solution()
    print(s.mapWordWeights(["abcd","def","xyz"], [5,3,12,14,1,2,3,2,10,6,6,9,7,8,7,10,8,9,6,9,9,8,3,7,7,2]))  # "rij"
    print(s.mapWordWeights(["a","b","c"], [1]*26))  # "yyy"
    print(s.mapWordWeights(["abcd"], [7,5,3,4]+[3]*22))  # "g"